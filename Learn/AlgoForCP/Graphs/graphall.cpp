...
BFS (For finding Height or Level)
...
1. Queue : When we use queue we explore all paths which is not a greedy approach which results in slower runtime and exploration of unnecessary paths.
2. Priority Queue : Priority queue is best Data Structure for implementing the Dijkstra Alogirthm in priority queue we have to do some extra extra exploration than set.
3. Set : Set erases the useless node which in case helps to reduce some extra iterations but removing element from set cost O(Logn) so overall there is not much difference in runtime when we use set or priority queue.
...
int spanningTree(int V, vector<vector> adj[])
{
	...
	while (!pq.empty())	{
		...
		if (from != to){
			...
			// MST.push_back({to, from}); // in case of undirected graph
		}
		...
		for (auto it : adj[to])
		{
			...
			if (!vis[childN])
			{
				pq.push({childW, {to, childN}});
			}
		}
	}

	// print MST edges if needed 
	// for(auto v : MST)
	//     cout << v[0] << " --> " << v[1] << endl;
